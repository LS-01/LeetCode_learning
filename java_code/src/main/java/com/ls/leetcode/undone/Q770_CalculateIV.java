package com.ls.leetcode.undone;
 ...
 */
public class Q770_CalculateIV {

    public List<String> basicCalculatorIV(String expression, String[] evalvars, int[] evalints) {
        if(evalvars.length != evalints.length || evalvars.length > 100 || expression.length() > 250 || expression.length() < 1){
            return null;
        }

        List<String> res = new ArrayList<>();

        Map<String, Integer> relaMap = new HashMap<>();
        for (int i = 0; i < evalvars.length; i++) {
            relaMap.put(evalvars[i], evalints[i]);
        }

        Stack<Object> sdata = new Stack<>();
        Stack<String> soper = new Stack<>();
        ...
        for (int i = 0; i < expression.length(); i++) {
            ...
            if(c >= 'a' && c <= 'z'){
                ...
                if(sb.length() > 0){
                    if(relaMap.containsKey(sb.toString())){
                        sdata.push(relaMap.get(sb.toString()));
                    } else {
                        sdata.push(sb.toString());
                    }
                } else if(n > -1){
                    sdata.push(n);
                }
        ...
        while (sdata.size() > 0){
            Object o = sdata.pop();
            System.out.print(o + "  ");
            System.out.print((o.getClass() == Integer.class) + "  ");
            System.out.print((o.getClass() == String.class) + "  ");
        }
        System.out.println();
        while (soper.size() > 0){
            System.out.print(soper.pop() + "  ");
        }
    ...
    public static void main(String[] args) {
        ...
        List<String> res = new Q770_CalculateIV().basicCalculatorIV(expression, evalvars, evalints);
        for (String s: res) {
